...
{
    public partial class SnackAddToWishlist : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (wishlistExist == 1)
            {
                ...
                Response.Write("<script language='javascript'>window.alert('Item Removed from Wishlist');window.location='Snack.aspx';</script>");
            }
            else
            {
                insertWishlist(customerID, itemID);

                Response.Write("<script language='javascript'>window.alert('Item Added to Wishlist!');window.location='Snack.aspx';</script>");
            }

            //Response.Redirect("Snack.aspx");
        }
