...
{
    public partial class PaintingAddToWishlist : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (wishlistExist == 1)
            {
                ...
                Response.Write("<script language='javascript'>window.alert('Art Removed from Wishlist');window.location='PaintingArt.aspx';</script>");
            }
            else
            {
                insertWishlist(customerID, artID);

                Response.Write("<script language='javascript'>window.alert('Art Added to Wishlist!');window.location='PaintingArt.aspx';</script>");
            }

            //Response.Redirect("PaintingArt.aspx");
        }
