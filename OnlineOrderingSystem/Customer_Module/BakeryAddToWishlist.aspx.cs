...
{
    public partial class BakeryAddToWishlist : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (wishlistExist == 1)
            {
                ...
                Response.Write("<script language='javascript'>window.alert('Item Removed from Wishlist');window.location='Bakery.aspx';</script>");
            }
            else
            {
                insertWishlist(customerID, itemID);

                Response.Write("<script language='javascript'>window.alert('Item Added to Wishlist!');window.location='Bakery.aspx';</script>");
            }

            //Response.Redirect("Bakery.aspx");
        }
